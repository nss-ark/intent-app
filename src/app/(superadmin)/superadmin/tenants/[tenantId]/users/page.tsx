...
export default function TenantUsersPage() {
  ...
  const { data, isLoading, error } = useQuery<TenantUsersResponse>({
    queryKey: ["superadmin-tenant-users", tenantId, page, search],
    queryFn: () =>
      apiFetch(
        `/api/superadmin/tenants/${tenantId}/users?page=${page}&pageSize=${pageSize}&search=${encodeURIComponent(search)}`
      ),
  });
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-[var(--intent-text-tertiary)] bg-white px-4 py-12 text-center">
          ...
          <p className="text-sm text-[var(--intent-text-secondary)]">
            {search ? "No users match your search." : "No users in this tenant."}
          </p>
        </div>
      ) : (
        <>
