...
function EventCard({
  ...
  const displayNiches = event.niches.slice(0, 2);
  const overflowCount = event.niches.length - 2;

  return (
    <div
      className="rounded-2xl border border-[var(--intent-text-tertiary)] bg-white p-5 transition-shadow hover:shadow-[var(--card-shadow)] cursor-pointer"
      role="button"
      tabIndex={0}
      onClick={() => router.push(`/spaces/${event.id}`)}
      onKeyDown={(e) =>
        e.key === "Enter" && router.push(`/spaces/${event.id}`)
      }
    >
      {/* Title + badges */}
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-[17px] font-semibold leading-snug text-[var(--intent-text-primary)]">
          {event.title}
        </h3>
      ...
      <p className="mt-2.5 flex items-center gap-1 text-[13px] text-[var(--intent-text-secondary)]">
        <Calendar size={13} strokeWidth={1.5} className="shrink-0" />
        <span>{format(new Date(event.startsAt), "EEE, MMM d 'at' h:mm a")}</span>
        {event.location && (
          <>
            <span className="mx-1 text-[var(--intent-text-tertiary)]">&middot;</span>
      ...
      {/* Creator + attendees — one line */}
      <div className="mt-3 flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          {event.creator && (
            <>
              <AvatarPlaceholder name={event.creator.name} size={22} />
              <span className="truncate text-[12px] text-[var(--intent-text-secondary)]">
                {event.creator.name}
              </span>
            </>
          )}
          <span className="text-[var(--intent-text-tertiary)]">&middot;</span>
          <span className="flex shrink-0 items-center gap-1 text-[12px] text-[var(--intent-text-secondary)]">
            <Users size={12} />
            {event.rsvpCount} going
          </span>
          {spotsLeft != null && spotsLeft > 0 && (
            <span className="shrink-0 text-[11px] text-[var(--intent-navy)]">
              {spotsLeft} left
            </span>
          )}
        </div>
      ...
      {/* Niche pills + RSVP — bottom row */}
      <div className="mt-3 flex items-center justify-between gap-3">
        <div className="flex flex-wrap gap-1.5 min-w-0">
          {displayNiches.map((n) => (
            <span
              key={n.id}
              className="rounded-full bg-[var(--intent-navy-subtle)] px-2.5 py-0.5 text-[10px] font-medium text-[var(--intent-navy)]"
            >
              {n.displayName}
            </span>
          ))}
        ...
        <div className="shrink-0" onClick={(e) => e.stopPropagation()}>
          {isAttending ? (
            <button
              ...
              disabled={isRsvping}
              className="flex h-8 items-center gap-1.5 rounded-lg bg-[var(--intent-green-subtle)] px-4 text-[12px] font-semibold text-[var(--intent-green)] transition-colors hover:bg-[var(--intent-green)]/20 disabled:opacity-50"
            >
              <Check size={13} />
              Attending
            </button>
            ...
            <button
              onClick={() => onUnRsvp(event.id)}
              disabled={isRsvping}
              className="flex h-8 items-center gap-1.5 rounded-lg bg-[var(--intent-navy-subtle)] px-4 text-[12px] font-semibold text-[var(--intent-navy)] transition-colors disabled:opacity-50"
            >
              Waitlisted
            </button>
          ) : (
            <button
              ...
              disabled={isRsvping}
              className="flex h-8 items-center gap-1.5 rounded-lg bg-[var(--intent-navy)] px-4 text-[12px] font-semibold text-white transition-colors hover:bg-[var(--intent-navy-light)] disabled:opacity-50"
            >
              Join
            </button>
          )}
